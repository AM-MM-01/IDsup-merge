...
def webhook():
    try:
        ...
        try:
            ...
            # После всех итераций добавляем тег merge в основной тикет
            if main_ticket_id:
                print(f"🏷️ Добавление тега 'merge' в основной тикет {main_ticket_id}...")
                if add_tags_to_ticket(main_ticket_id, ["merge"]):
                    print(f"✅ Тег 'merge' успешно добавлен в тикет {main_ticket_id}.")
                else:
                    print(f"⚠️ Не удалось добавить тег 'merge' в тикет {main_ticket_id}.")
            else:
                print("ℹ️ Не было основного тикета для добавления тега 'merge'.")
